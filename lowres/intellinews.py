...
def generate_document_analysis(Q, df, llm):
    ...
    joined_chunks = sorted_df.groupby('title')['documents'].apply(' '.join).reset_index()
    titles = joined_chunks['title'].to_list()    
    documents = joined_chunks['documents'].to_list()
    doc_input = ''
    for i in range(len(joined_chunks)):
        doc_input += f"""
        Document {titles[i]} Title: {titles[i]}
        Document {titles[i]} Content: {documents[i]}
        """
    ...
    if len(df) < 2:
        ...
        if btn_ask and Q.strip() != '':
            if len(QDOCS) <= 1:
                ...
                joined_chunks = results_df.groupby('title')['documents'].apply(' '.join).reset_index()
                ...
                with st.expander("Semantic Data Analysis:", expanded=True):
                    ...
                    st.subheader(f'Semantic Search Results Data Joined:')
                    st.dataframe(joined_chunks)
